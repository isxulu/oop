...
template<class T>
using allo = MyAllocator<T>; // replace the std::allocator with your allocator
...
int main()
{
  ...
  // vector creation
  using IntVec = std::vector<int, allo<int>>;
  std::vector<IntVec, allo<IntVec>> vecints(TestSize);
  for (int i = 0; i < TestSize; i++)
    vecints[i].resize(dis(gen));

  using PointVec = std::vector<Point2D, allo<Point2D> >;
  std::vector<PointVec, allo<PointVec>> vecpts(TestSize);
  for (int i = 0; i < TestSize; i++)
